...
import { toEmail } from '../lib/validators'
...
export default function ResetPasswordPage() {
  ...
  const handleReset = async (e: FormEvent) => {
    ...
    setBusy(true)
    try {
      await sendPasswordResetEmail(auth, toEmail(id))
  ...
  return (
    <div className="center-page">
      <form className="auth-card" onSubmit={handleReset}>
        <h2>비밀번호 재설정</h2>
        <div className="sub">가입한 이메일로 재설정 링크를 보내드립니다.</div>

        <div className="field">
          <label htmlFor="rp-id">아이디 (이메일)</label>
